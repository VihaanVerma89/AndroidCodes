...
public class CartManager {
    ...
    {
        addToCart(item.getItemId(), item.getName(), item.getType(), item.getSize(), item.getCost(), item.getQuantity());
    }

    public void removeFromCart(Item item)
    {
        removeFromCart(item.getItemId(), item.getName(), item.getType(), item.getSize(), item.getCost(), item.getQuantity());
    ...
    public void removeFromCart(String itemId, String name, String type, String size, String cost, String quantity) {

        if(presentInCart(itemId, cost))
        {
            decreaseQuantity(itemId, cost);
        }
    ...
    public void decreaseQuantity(String itemId, String cost)
    {
        ContentValues values = new ContentValues();
        String quantity = getQuantity(itemId, cost);
        int temp = Integer.parseInt(quantity);
    ...
    public void deleteItem(String itemId, String cost)
    {
//        mSqLiteDatabase.execSQL("delete from " + Tables.Cart.TABLE_NAME + " where " + Tables.Cart.ITEM_ID + " = " + itemId + " and " + Tables.Cart.COST + " = " + cost +" ;" , null);
//        mSqLiteDatabase.execSQL("delete from " + Tables.Cart.TABLE_NAME + " where " + Tables.Cart.ITEM_ID + " = " + itemId + " and " + Tables.Cart.COST + " = " + cost +" ;" , null);
          mSqLiteDatabase.delete(Tables.Cart.TABLE_NAME, Tables.Cart.ITEM_ID + " = ?  and  "+ Tables.Cart.COST + " = ?" , new String [] {itemId, cost});
    }
